...
- 성경 데이터를 체계적으로 정리하고, 이름을 생성하는 AI 처리 과정 설계
- 이름의 성경적 근거를 확인하는 기능과 발음이 자연스러운지 평가하는 기능 구현
- Docker·GitHub Actions로 자동 배포 환경과 서버 운영 설정 구성
- Google·Naver·Kakao 소셜 로그인 구현',
  ...
  true,
  '- 기존 작명 방식의 한계: 사주나 음양오행으로 짓는 작명 서비스는 흔하지만, 성경 말씀에서 의미를 찾아 이름을 짓는 서비스는 접해보지 못해 직접 만들어보기로 함
