...
values (
  ...
  'GitHub 저장소 관리자로 6인 팀 브랜치 통합 총괄, 글 생성 플로우·OAuth2 소셜로그인 3사 통합·FastAPI 연동 설계, 관리자 페이지 프론트엔드 구현. 이후 Docker 컨테이너화·GitHub Actions CI/CD·RAG 구조 설계·nginx/HTTPS 배포는 개인적으로 이어서 진행',
  array['3계층 아키텍처 통합 — React ↔ Spring Boot ↔ FastAPI 구조에서, Spring이 FastAPI를 동기 호출하는 전용 클라이언트를 만들고 {success, data, error} 공통 응답 형식으로 성공/실패를 구분하도록 두 서버 간 규칙을 맞춰 실제로 동작하게 연결', '단계별 글 생성 플로우 — 키워드 분석 → 본문 생성 → 제목 키워드 분석 → 제목 생성, 4단계 흐름을 하나의 로직에서 단계별로 나눠 처리. 임시 저장 테이블에 매 단계 입력값·생성 결과·현재 단계를 저장해 중간에 나갔다 와도 이어서 작업할 수 있도록 설계, 완성되면 최종 게시물로 전환. 프론트엔드 단계별 화면(1~3단계+완성)과 진행 상태 표시도 직접 설계', 'OAuth2 소셜로그인 3사 통합 — Google/Naver/Kakao 응답 구조가 제각각(Kakao는 프로필 정보가 한 번 더 중첩되고, Naver는 응답 값이 한 번 더 감싸진 구조)인 걸 공통 형태로 통일한 뒤, 플랫폼별로 세부 처리만 따로 나눠서 처리하도록 설계. JWT 발급/Redis 리프레시 토큰 저장은 팀원과 공동 작업 영역', 'Docker 컨테이너화 — Spring Boot/FastAPI/React 3개 서비스 전부 Dockerfile 직접 작성(FastAPI는 프로덕션 의존성만 분리, React는 멀티스테이지 빌드로 빌드 후 nginx로 서비스). MySQL/Redis 포함 docker-compose로 로컬 전체 스택을 한 번에 기동하도록 구성', 'Oracle Cloud VPS 배포 — RAM 1GB짜리 완전 초기 상태 VPS(Ubuntu 24.04)에 Docker 설치, 2GB 스왑 설정, GitHub 컨테이너 레지스트리에 이미지 3개 푸시, 운영용 설정(JVM 힙 제한, 불필요 포트 비공개)으로 배포. 클라우드 방화벽과 서버 내부 방화벽 양쪽을 다 열어야 접속된다는 것까지 직접 확인', 'GitHub Actions CI/CD — 3개 저장소 각각에 이미지 빌드→레지스트리 푸시→서버 배포까지 자동화하는 워크플로우 작성. 3개 저장소 모두 push 한 번으로 자동 배포되는 것 확인 완료', 'RAG(검색증강생성) 구조 설계 및 구현 — 톤(tone)마다 예문 하나만 고정으로 넣던 프롬프트 구조를, 요청 내용(상품 정보·키워드)과 의미적으로 가장 비슷한 예문을 자동으로 찾아 넣는 구조로 전 구간(백엔드/AI 서버/프론트) 재설계. 별도 벡터DB 없이 OpenAI 임베딩 API + 코사인 유사도 계산만으로 경량 구현(1GB RAM VPS 제약 고려), 예문을 저장하는 테이블과 관리 API를 새로 만들고 예문 등록/삭제 시 실시간으로 유사도 계산이 갱신되도록 처리. 캠핑/주방/뷰티 카테고리 예문으로 검색 정확도 검증(관련 예문 유사도 0.33, 무관 예문 0.19~0.24로 명확히 구분됨을 확인)', '도메인 + nginx 리버스 프록시 + HTTPS(Let''s Encrypt) — 도메인 구매 후 nginx를 프록시 계층으로 세워 API 요청은 백엔드로, 나머지는 프론트엔드로 라우팅하도록 재구성하고 백엔드/프론트 포트 직접 노출 제거. Certbot으로 무료 SSL 인증서 발급, HTTP→HTTPS 자동 리다이렉트, 인증서 자동 갱신까지 무중단 구성'],
  '[{"title":"FastAPI 연동 트러블슈팅","problem":"Postman으로 FastAPI 엔드포인트를 테스트하던 중 Spring이 보낸 JSON 값이 전달되지 않는 문제 발견","solution":"엔드포인트 파라미터가 Form 방식(form-urlencoded)으로 선언돼 있어 JSON 형식 데이터를 못 받아들이던 것이 원인임을 진단, 관련 엔드포인트 5개를 모두 JSON 바디를 받도록 수정해 해결"},{"title":"GHCR 이미지 푸시 권한 오류","problem":"GitHub Actions에서 빌드한 이미지를 이미지 레지스트리에 푸시할 때 계속 거부됨","solution":"저장소의 Actions 권한을 읽기 전용에서 읽기/쓰기로 바꾸고, 패키지 소유 계정 쪽 접근 권한도 함께 열어줘야 한다는 걸 단계적으로 원인을 좁혀가며 해결"},{"title":"배포 인프라 오류 3건","problem":"사용하던 베이스 이미지가 배포 저장소에서 삭제됨 / 환경변수 값 형식 오류로 디코딩 실패 / 프론트엔드 API 주소 설정 누락으로 인증 오류 발생","solution":"각각 대체 이미지 교체, 올바른 값으로 재설정, 누락된 설정값 추가로 해결"},{"title":"HTTPS 전환 후 Mixed Content 오류","problem":"HTTPS 전환 직후 브라우저가 보안 정책 위반으로 API 호출을 차단","solution":"프론트엔드가 빌드 시점에 저장해둔 API 주소가 예전 값이었던 것이 원인 — 새 주소 기준으로 다시 빌드해 해결"},{"title":"GitHub Actions 배포 실패 (Service Unavailable)","problem":"재배포 중 GitHub Actions가 일시적으로 실패","solution":"GitHub 쪽 일시 장애로 판단, 재실행으로 해결"}]'::jsonb,
  true,
...
values (
  ...
  2,
  '[{"url":"https://pansfwryctxokvmssywb.supabase.co/storage/v1/object/public/portfolio-images/projects/1785771463765.png","type":"image"},{"url":"https://pansfwryctxokvmssywb.supabase.co/storage/v1/object/public/portfolio-images/projects/1785771445061.png","type":"image"},{"url":"https://pansfwryctxokvmssywb.supabase.co/storage/v1/object/public/portfolio-images/projects/1785771466814.png","type":"image"}]'::jsonb,
  ...
  array['계층형 퍼지 상태머신(HFSM) — 체력을 낮음/중간/높음, 거리를 가까움/중간/먼 3단계로 나누고, 규칙별 계산식으로 점수를 매겨 가중평균을 내는 방식(Sugeno 퍼지 추론)으로 상위 상태(대기/접근/전투/광폭) 결정 후 거리 기준 하위 상태를 고르는 2단계 구조로 설계 — 근접형/원거리형/슬로우형/일벌형/중형 보스 등 9개 이상 몬스터 종류가 공유하는 실제 프로덕션 로직', '모방학습(imitation learning) 파이프라인 — 퍼지 시스템이 실제 플레이 중 만들어낸 (거리·각도·체력·속성관계·오염도 → 행동) 데이터를 매 프레임 정규화해 기록 → 좌표를 회전각 기반 값으로 변환해 각도 계산의 불연속 문제를 해결 → PyTorch 2-layer LSTM(32→16 hidden, Dropout 0.5)으로 학습 → ONNX 형식으로 변환 → Unity에서 0.1초 간격으로 실시간 추론, 확률 계산으로 행동을 결정', '데이터 전처리 튜닝 — 전처리 코드의 기본 틀은 수업에서 제공된 것이었고, 게임 데이터에 맞춘 튜닝(불필요한 열 제거, 3D 거리 값 정규화, 데이터를 일정 간격으로 나눠 과적합을 방지하는 처리)은 직접 작업', '보스 몬스터 페이즈 시스템 — 체력 기준 3단계 페이즈 × 4가지 공격 패턴(수비/균형/공격/광폭) 조합, 페이즈가 바뀔 때마다 이벤트로 상태 변화를 알리는 방식, 여왕벌이 일벌 부대를 지휘하는 구조로 설계'],
...
values (
  ...
  array['다중 조건 매매 전략 — MA10/30 골든크로스 또는 추세유지 + ADX>25(상승중) + RSI<60 + 거래량 필터(20봉 평균×1.3) + 4시간봉 상위 타임프레임 필터. 여러 버전을 반복 실험하며 파라미터를 조정해 가장 안정적인 조합을 채택', '리스크 관리 — ATR×1.5 손절(최대 −3% 캡) / ATR×3.5 익절(손익비 목표 1:2.3) / 트레일링스탑(+2% 활성화, 최고가 대비 −2% 청산), 종목별 보유 자산을 슬롯 수로 나눠 배분하는 멀티 포지션 구조', '패턴 감지·신호 스코어링 — 골든/데드크로스, ADX 강도, RSI 과매수/과매도, 볼린저밴드 이탈/지지, 거래량 급증 등 11가지 패턴을 감지해 0~100점 신호 스코어로 환산, 규칙 기반 시황 코멘트를 자동 생성해 Discord로 전송'],
