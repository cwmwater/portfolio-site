...
values (
  ...
  array['다중 조건 매매 전략 — MA10/30 골든크로스 또는 추세유지 + ADX>25(상승중) + RSI<60 + 거래량 필터(20봉 평균×1.3) + 4시간봉 상위 타임프레임 필터 결합. 여러 버전을 반복 실험하며 파라미터를 조정해 가장 안정적인 조합 채택.', '리스크 관리 — ATR×1.5 손절(최대 −3% 캡) / ATR×3.5 익절(손익비 목표 1:2.3) / 트레일링스탑(+2% 활성화, 최고가 대비 −2% 청산) 적용. 종목별 보유 자산을 슬롯 수로 나눠 배분하는 멀티 포지션 구조로 운용.', '패턴 감지·신호 스코어링 — 골든/데드크로스, ADX 강도, RSI 과매수/과매도, 볼린저밴드 이탈/지지, 거래량 급증 등 11가지 패턴을 감지해 0~100점 신호 스코어로 환산. 규칙 기반 시황 코멘트를 자동 생성해 Discord로 전송.'],
